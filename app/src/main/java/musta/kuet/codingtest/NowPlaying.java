...
public class NowPlaying extends Activity implements View.OnClickListener, MediaPlayer.OnCompletionListener {
    ...
    SeekBar bar;
    Button preSong, nextSong, playPause, goToSongList, shuffle, repeat, nowPlayingSongs;
    ...
    public void openActivityNotification() {
        ...
        Intent notifyIntent = new Intent(this, PlayingActivity.class);
       ...
       builder.setContentTitle("Musician");
       builder.setContentText("Touch to Musician");

        nm.notify(99991,builder.build());
    }
...
//				break;
            ...
            case R.id.songList:
                ...
                intent.setClass(NowPlaying.this, PlayingActivity.class);
...
//                }
            ...
            case R.id.imageViewSongList:
                ...
                final ListView listView = (ListView) dialog.findViewById(R.id.songsDialog);
                listView.setAdapter(adapter);
//                final EditText searchable = new EditText(NowPlaying.this);
...
//                searchable.setLayoutParams(params);
//                dialog.setContentView(searchable);
//                final EditText searchable = (EditText) findViewById(R.id.search_bar_dialog);
//                searchable.addTextChangedListener(new TextWatcher() {
//                    @Override
//                    public void beforeTextChanged(CharSequence s, int start, int count, int after) {
//                        try {
//                            NowPlaying.this.adapter.getFilter().filter(s);
...
//                        //seekBarUpdating.start();
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        if (isRepeatOneOn == true){
            position = currentSongPosition;
        }
        if (shuffleOn == true && isRepeatOn == true){
            position = randomPosition.nextInt((songs.size() - 0) + 0);
        }
        uri = Uri.parse(songs.get(position).toString());
        try {
    ...
    private void updateSongInfoFromDialog(int positionFromDialog) {
        ...
        currentSong.setText("" + songs.get(positionFromDialog).getName().replace(".mp3", "").replace(".MP3", "").replace(".wav", "").replace(".WAV", "").replace("_", " ").trim());
        //.replace("_", " ")
    }
}
